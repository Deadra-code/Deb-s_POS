        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            {loading ? (
                ...
                <div className="divide-y divide-slate-100">
                    {list.length === 0 && <div className="p-8 text-center text-slate-400">Tidak ada transaksi pada tanggal ini.</div>}
                    {list.map((t, idx) => (
                                ...
                                {t.items.map((i, ii) => (
                                    <div key={ii} className="flex justify-between text-xs">
                                        <span className="text-slate-600">
                                            {i.qty}x {i.nama}
                                            <span className="text-[10px] text-slate-400 italic ml-1">({i.milik})</span>
                                        </span>
