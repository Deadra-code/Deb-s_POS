    ...
    return (
        <div className="relative overflow-hidden mb-4 rounded-2xl">
            ...
            <motion.div
                ...
                className="absolute inset-0 bg-red-500 rounded-2xl flex items-center justify-end pr-6 z-0 overflow-hidden"
            >
            ...
            <motion.div
                ...
                className="relative z-10 bg-white dark:bg-slate-900 flex justify-between items-center p-4 rounded-2xl border border-slate-50 dark:border-slate-800 shadow-sm overflow-hidden"
            >
                <div className="flex-1 min-w-0 pr-4">
                    ...
                    {variants.length > 0 && (
                        <div className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wide">
                            {variants.map((v, idx) => <span key={idx}>{v.name.includes(':') ? v.name.split(':')[1].trim() : v.name}</span>)}
                        </div>
                    )}
                    ...
                    {addons.length > 0 && (
                        <div className="text-[10px] text-blue-500 dark:text-blue-400 italic">
                            + {addons.map((m, idx) => <span key={idx}>{m.name.replace('Tambah ', '')}</span>)}
                        </div>
                    )}
