...
const Inventory = ({ menu, refreshData, loading: parentLoading }) => {
    ...
    return (
        <main className="p-4 md:p-8 h-full overflow-y-auto pb-24 text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-950 transition-colors duration-300 relative">
            ...
            {localLoading && <div className="fixed inset-0 z-40 bg-white/50 dark:bg-slate-950/50 flex items-center justify-center"><Icon name="loader" className="animate-spin text-emerald-600" size={40} /></div>}
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={handleAdd} className="bg-emerald-600 text-white px-4 py-2.5 rounded-xl font-bold flex gap-2 shadow-lg hover:bg-emerald-700 active:scale-95 transition-transform shrink-0" aria-label="Tambah Produk Baru"><Icon name="plus" size={20} /> Produk Baru</button>
            </div>
                ...
                <section className="md:hidden space-y-3">
                    {isLoading ? (
                        Array(5).fill(0).map((_, i) => (
                            <div key={i} className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-md border border-slate-100 dark:border-slate-800 flex gap-4 overflow-hidden">
                        ...
                        menu.map((item, idx) => (
                            <article key={item.ID || idx} className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-md border border-slate-100 dark:border-slate-800 flex gap-4 transition-all active:scale-[0.98] overflow-hidden">
