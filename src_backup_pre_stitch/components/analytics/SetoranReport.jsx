        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                    ...
                    <div className="text-2xl font-bold text-slate-800 dark:text-slate-100">Rp {(stats?.cashInDrawer || 0).toLocaleString('id-ID')}</div>
                </div>
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-900/30">
                    ...
                    <div className="text-2xl font-bold text-slate-800 dark:text-slate-100">Rp {(stats?.digitalBank || 0).toLocaleString('id-ID')}</div>
                </div>
            </div>
            <div className="mt-4 pt-4 border-t border-dashed border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-4">
                {Object.entries(stats?.ownerTotals || {}).map(([owner, amount]) => (
                    <div key={owner}>
                        <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase">Setoran {owner}</div>
                        <div className="font-bold text-slate-700 dark:text-slate-200">Rp {(amount || 0).toLocaleString('id-ID')}</div>
                    </div>
                ))}
            </div>
        </div>
